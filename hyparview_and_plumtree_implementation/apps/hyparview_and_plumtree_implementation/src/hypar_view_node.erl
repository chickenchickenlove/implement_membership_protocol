...
-record(?MODULE, {name = undefined :: atom(),
                  ...
                  gossip_protocol_module = undefined :: module(),
                  gossip_protocol_state = undefined
}).
...
%%  process_flag(trap_exit, true),
  ...
  hypar_view_node:start_link('A', Config, plum_tree_protocol, hypar_erl_reactor),
  timer:sleep(1000),

  hypar_view_node:start_link('B', Config, plum_tree_protocol, hypar_erl_reactor),
  timer:sleep(1000),

  hypar_view_node:start_link('C', Config, plum_tree_protocol, hypar_erl_reactor),
  timer:sleep(1000),

  hypar_view_node:start_link('D', Config, plum_tree_protocol, hypar_erl_reactor),
  timer:sleep(1000),

  hypar_view_node:start_link('E', Config, plum_tree_protocol, hypar_erl_reactor),
  timer:sleep(1000),
...
start_link(NodeName, Config, GossipProtocolModule, ReactorModule) ->
  gen_server:start_link(?MODULE, [NodeName, Config, GossipProtocolModule, ReactorModule], []).
...
init([NodeName, Config, GossipProtocolModule, ReactorModule]) ->
  SelfPid = self(),
  ...
  State = #?MODULE{
    ...
    gossip_protocol_module=GossipProtocolModule,
    gossip_protocol_state=GossipProtocolState,
    reactor=ReactorModule},
  {ok, State}.
...
handle_cast({{shuffle, OriginNodeName, ReceivedShuffledNodes, TTL}, _FromPid}, State0) ->
...
handle_cast(Msg, State0) ->
  #?MODULE{gossip_protocol_state=GossipProtocolState0, gossip_protocol_module=GossipProtocolModule} = State0,
  GossipProtocolState = GossipProtocolModule:handle_message(Msg, GossipProtocolState0),
  State = State0#?MODULE{gossip_protocol_state=GossipProtocolState},
  {noreply, State}.
...
add_passive(PeerName, State0) ->
  #?MODULE{active_view=ActiveView, passive_view=PassiveView0} = State0,
